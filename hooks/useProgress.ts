...
export function useProgress(userId: string | null, taxpayerProfileId: string | null = null) {
  ...
  async function calculateProgress() {
    ...
    try {
      ...
      // profileFilter: soporta multi-contribuyente cuando taxpayerProfileId está disponible
      ...
      const [taxpayerRes, stepsRes, regimeRes, fiscalAddrRes, posRes] = await Promise.all([
        ...
        (supabase.from('taxpayer_regime_status') as any).select('status, tax_regimes(code)').eq(profileFilter.col, profileFilter.val).eq('status', 'active'),
      ...
      const activeRegimes = (regimeRes.data || []) as any[]
      ...
      const hasEFiscalAddress = (fiscalAddrRes.data as any)?.status === 'constituted'
  ...
  useEffect(() => { calculateProgress() }, [userId, taxpayerProfileId])
